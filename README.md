...
```
[00:00:00.000,000] <inf> hello_inference: === SynapticOS 0.5.0 — Hello Inference ===
...
│   │                       flash map/port, IPC, boot, health/watchdog, DMA ingest,
│   │                       profiling, init, shell
│   ├── hal/
...
- **The DMA driver may not know your transfer mode** (Phase 5). Zephyr 3.7's eDMA driver never issues the software START that mem-to-mem transfers need on eDMA v4 (and arms the hardware request on mux source 0 instead). The HAL programs the transfer itself &mdash; including clearing the latched DONE flag, which is write-1-to-clear and silently gates the next START.
...
- **A preempting submitter must outrank the scheduler thread** (Phase 5). The on-hardware preemption demo failed on first run because the submitting thread could never inject mid-job from a lower priority &mdash; QEMU's cooperative test threads had masked the requirement entirely.
